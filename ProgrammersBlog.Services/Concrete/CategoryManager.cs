...
namespace ProgrammersBlog.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        {
            var categories = await unitOfWork.Categories.GeltAllAsync(null);
        ...
        {
            var categories = await unitOfWork.Categories.GeltAllAsync(c => !c.IsDeleted);
            if (categories.Count > -1)
            {
                return new DataResult<CategoryListDto>(ResultStatus.Success, new CategoryListDto
                {
        ...
        {
            var categories = await unitOfWork.Categories.GeltAllAsync(c => c.IsActive && !c.IsDeleted);
            if (categories.Count > -1)
            {
                return new DataResult<CategoryListDto>(ResultStatus.Success, new CategoryListDto
                {
